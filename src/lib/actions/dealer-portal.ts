...
/**
 * 로그인한 딜러 본인 기준으로 dealerId를 알아내서 priceCartItemsForDealer를
 * 부른다. 새 주문 생성(placeDealerCartOrder)과 딜러 자신의 draft 주문 수정
 * (updateDealerCartOrder)이 이 함수를 함께 쓴다. 실제 가격 계산 로직 자체는
 * lib/utils/dealer-cart-pricing.ts에 있어서, 관리자가 딜러 주문을 대신
 * 고치는 액션(dealer-order-admin.ts)도 그 파일에서 그대로 가져다 쓴다 —
 * "use server" 파일에 두면 모든 export가 서버 액션(async 함수)이어야
 * 한다는 Next.js 제약 때문에, 순수 유틸은 별도 파일로 뺐다.
 */
